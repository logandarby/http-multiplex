...
int start(const int port, const char *resources_path,
  ...
  while (*is_running) {
    ...
    for (size_t i = 0; i <= fdpool.max_fd && nready > 0; i++) {
      ...
      if (fd_type == FdDataType_SERVER) {
        ...
        const int client_fd =
            file_module->accept(server.fd, NULL, NULL);
        if (client_fd == -1) {
        ...
        if (str_eq(method, "GET", BUFFER_SIZE)) {
          ...
          if (fd_to_read == -1) {
            file_module->write(client_fd, NOT_FOUND_ERR,
                               strlen(NOT_FOUND_ERR));
      ...
      close_client_connection:
        ...
        const size_t bytes_read =
            file_module->read(file_fd, buffer, BUFFER_SIZE);
        if (bytes_read < 0) {
  ...
  DZ_ASSERT(fdpool_size(&fdpool) == 0);
  fdpool_free(&fdpool);
  return EXIT_SUCCESS;
}
