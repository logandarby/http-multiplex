...
void fdpool_add_fd(FdPool *self, const int fd, enum FdDataType type) {
  ...
  const int cntlerr = fcntl(fd, F_SETFD, O_NONBLOCK);
  if (cntlerr == -1) {
    DZ_ERRORNO("Cannot fctl fd");
  }
...
void fdpool_set_file_data(FdPool *self, int fd_to_read,
  ...
  DZ_ASSERT(self);
  const bool is_file_data_type =
      self->fd_data[fd_to_read].data_type == FdDataType_FILE;
  DZ_ASSERT(is_file_data_type,
            "Cannot set file data for file descriptor which is not "
            "FdDataType_FILE");
...
FdDataFile fdpool_get_file_data(FdPool *self, int fd_to_read) {
  ...
  const bool is_file_data_type =
      self->fd_data[fd_to_read].data_type == FdDataType_FILE;
  DZ_ASSERT(is_file_data_type,
            "Cannot get file data from file descriptor which is not "
            "FdDataType_FILE");
